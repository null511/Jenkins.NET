...
namespace JenkinsNET
{
    ...
    public sealed class JenkinsClientBuilds
    {
        ...
        /// <param name="token">An optional token for aborting the request.</param>
        /// <exception cref="JenkinsNetException"></exception>
        public async Task<string> GetConsoleTextAsync(string jobName, string buildNumber, CancellationToken token = default(CancellationToken))
        {
            try {
                var cmd = new BuildTextCommand(context, jobName, buildNumber);
                await cmd.RunAsync(token);
        ...
        /// <param name="token">An optional token for aborting the request.</param>
        /// <exception cref="JenkinsNetException"></exception>
        public async Task<string> GetConsoleHtmlAsync(string jobName, string buildNumber, CancellationToken token = default(CancellationToken))
        {
            try {
                var cmd = new BuildHtmlCommand(context, jobName, buildNumber);
                await cmd.RunAsync(token);
